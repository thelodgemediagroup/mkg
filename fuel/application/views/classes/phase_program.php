
	<!-- begin Content -->
	<section id="single-trainer" >
		<!-- begin Sub Header -->
		<div class="sub-header" >
			<div class="container">
				<div class="row" >
					<ul class="sub-header-container" >
						<li>
							<h3 class="title">MKGD PHASE Program</h3>
						</li>
						<li>
							<ul class="custom-breadcrumb" >
								<li><h6><a href="<?php echo base_url(); ?>">Home</a></h6></li>
								<li><i class="separator entypo-play" ></i></li>
								<li><h6><a href="<?php echo base_url('classes'); ?>">Classes</a></h6></li>
								<li><i class="separator entypo-play" ></i></li>
								<li><h6>Phase Program</h6></li>
							</ul>                    
						</li>
					</ul>
				</div>
			</div>
		</div>
		<!-- end Sub Header -->


		<!-- begin Trainer Profile -->
		<article class="article-container">
			<div class="container" >
				<div class="row" >
					<div class="col-md-12">
					<h2 class="article-title" >PHASE Program</h2>                  
					<span class="line" >
						<span class="sub-line" ></span>
					</span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-5 col-md-3">
					<figure>
						<img src="<?php echo img_path('classes/01.jpg'); ?>" alt="image">
					</figure>
					</div>
					<div class="sol-sm-7 col-md-5">
						<p>The MKG Phase program is a progressive “mixed art” program.  It is our signature method, and we recommend it to all our students, regardless of their martial arts experience.  You will improve your fitness, self-discipline, coordination and self defense capabilities, while you relieve stress in a social and non-competitive environment. </p>
						<p>With six phases of development encompassing much of the MKG Martial Arts material, we recommend both the curious beginner and the seasoned martial artist begin in our Phase One class. Phase One is a blend of curriculum, combining material primarily from Muay Thai kickboxing, Filipino Kali, and Jun Fan Gung Fu/Jeet Kune Do. Phase One is MKG’s most popular class and is the signature training and personal-development method applied at all MKG Branch schools. Class material aims to promote a foundation in cardiovascular conditioning and proper body mechanics, develop an effective defensive structure, and the ability to generate an offense.  Classes are fun and high-energy, leaving you with a good sweat and a smiling face.</p>
					</div>
					<div class="col-md-4">
						<div class="panel panel-gym">
							<div class="panel-heading">
								<h3 class="panel-title">Class Details</h3>
							</div>
							<div class="panel-body">
								<ul class="custom-icon-list" >
									<li><i class="entypo-user" ></i>Teacher: Joe Smith</li>
									<li><i class="entypo-calendar" ></i>Monday to Friday</li>
									<li><i class="entypo-clock" ></i>10AM - 11AM and 7PM - 8PM</li>
									<li><i class="entypo-location" ></i>Room 07, Center Plaza 15654</li>
									<li><i class="entypo-newspaper" ></i>See Pricing Tables</li>
								</ul>
							</div>
							<div class="panel-footer"><a href="<?php echo base_url('registration'); ?>">Registration</a></div>
						</div>
					</div>
				</div>
			</div>
		</article>
		<!-- end Trainer Profile -->

		<!-- begin Fitness Education -->
		<article class="article-container">
			<div class="container" >
				<div class="row">
					<div class="col-md-6">
						<h2 class="article-title">Fitness Education</h2>
						<span class="line" >
							<span class="sub-line" ></span>
						</span>

						<h3>Do you have a well-planned fitness routine?</h3>
						<p>Proin eros ante, viverra id risus non, porttitor posuere enim. In hac habitasse platea dictumst. Integer dapibus velit eu mattis ullamcorper. Nam ac placerat ligula, eu sodales nisl. Sed libero nisi, aliquet non tellus non, pharetra faucibus diam. Phasellus scelerisque fringilla diam, ac rhoncus purus sodales mattis. Nullam eleifend molestie felis ac tempus. Curabitur semper arcu a est viverra, vel adipiscing dui tristique. </p>
						<p>Maecenas ut scelerisque ante. Maecenas nunc libero, suscipit ut adipiscing sit amet, accumsan sed eros. Vestibulum varius lacus eget molestie tempor. Nam bibendum ullamcorper laoreet. Sed vestibulum erat ac est facilisis commodo. </p>
					</div>
					<div class="col-md-6">
						<h2 class="article-title">Read FAQ</h2>
						<!-- begin Accordion -->
						<div class="accordion" id="accordion2">
							<div class="accordion-group">
								<div class="accordion-heading">
									<h5>1. when I should start?</h5>
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseOne">+</a>
								</div>
								<div id="collapseOne" class="accordion-body collapse in">
								<div class="accordion-inner">
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseOne">-</a>
									<h5>our coaching answer:</h5>
									<p>
										Ut a molestie urna. Vestibulum malesuada, diam ut mollis varius, lorem urna euismod purus, ut varius est erat a orci. Sed ultricies elit eget venenatis tristique. Sed tempus egestas ante, vel ullamcorper nulla ultricies at. Maecenas pellentesque rhoncus egestas. Aenean molestie nisl eget semper pretium. Praesent adipiscing faucibus magna, eu vulputate ante volutpat volutpat and ulla facilisi.
									</p>
								</div>
								</div>
							</div>
							
							<div class="accordion-group">
								<div class="accordion-heading">
									<h5>2. how hard is Yoga Pilates?</h5>
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseTwo">+</a>
								</div>
								<div id="collapseTwo" class="accordion-body collapse">
								<div class="accordion-inner">
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseOne">-</a>
									<h5>Indoor Cycling</h5>
									<p>
										Ut a molestie urna. Vestibulum malesuada, diam ut mollis varius, lorem urna euismod purus, ut varius est erat a orci. Sed ultricies elit eget venenatis tristique. Sed tempus egestas ante, vel ullamcorper nulla ultricies at. Maecenas pellentesque rhoncus egestas. Aenean molestie nisl eget semper pretium. Praesent adipiscing faucibus magna, eu vulputate ante volutpat volutpat and ulla facilisi.
									</p>
								</div>
								</div>
							</div>

							<div class="accordion-group">
								<div class="accordion-heading">
									<h5>3. is Cardio Fitness a hard work?</h5>
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseThree">+</a>
								</div>
								<div id="collapseThree" class="accordion-body collapse">
								  <div class="accordion-inner">
									<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapseOne">-</a>
									<h5>Indoor Cycling</h5>
									<p>
										Ut a molestie urna. Vestibulum malesuada, diam ut mollis varius, lorem urna euismod purus, ut varius est erat a orci. Sed ultricies elit eget venenatis tristique. Sed tempus egestas ante, vel ullamcorper nulla ultricies at. Maecenas pellentesque rhoncus egestas. Aenean molestie nisl eget semper pretium. Praesent adipiscing faucibus magna, eu vulputate ante volutpat volutpat and ulla facilisi.
									</p>
								</div>
								</div>
							</div>
						</div>
					</div>    
				</div>
			</div>
		</article>
		<!-- end Fitness Education -->
	</section>
	<!-- end content -->
